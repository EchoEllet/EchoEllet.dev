...
# **This site is still under active development, and the blog posts are not yet available.**
...
<!-- Changes are not done, decide the fate of models related to MinecraftVersionManifest, move MinecraftAccount and MinecraftAccounts away from data to logic, update the logic layer instructions and what might they contain from data classes,
documenting that models returned from the data sources usually stay close or same as
the data source, where models of logic/domain includes get mapped from data models and
only include relevant fields. Fully review current related MinecraftVersionManifest models -->
...
* **Define the mappers that map source models to app models and vice versa.**
